...
namespace Infrastructure.ExelExplorer
{
    public class ExelExplorer : IExelExplorer
    {
        ...
        public Tuple<CorrectionFactors, Limitations, ExhaustiveSearchFactors, int> Import(
            string fileName
        )
        {
            ...
            //
            limitations.MinT1 = Convert.ToDouble(ObjWorkSheet.Cells[2, 4].Text.ToString());
            limitations.MaxT1 = Convert.ToDouble(ObjWorkSheet.Cells[3, 4].Text.ToString());
            limitations.MinT2 = Convert.ToDouble(ObjWorkSheet.Cells[4, 4].Text.ToString());
            limitations.MaxT2 = Convert.ToDouble(ObjWorkSheet.Cells[5, 4].Text.ToString());
            limitations.MinDiff = Convert.ToDouble(ObjWorkSheet.Cells[6, 4].Text.ToString());
            //
            exhaustiveSearchFactors.Eps = Convert.ToDouble(
                ObjWorkSheet.Cells[2, 6].Text.ToString()
            );
            exhaustiveSearchFactors.Step = Convert.ToDouble(
                ObjWorkSheet.Cells[3, 6].Text.ToString()
            );
            ObjWorkBook.Close(false, Type.Missing, Type.Missing);
            return new Tuple<CorrectionFactors, Limitations, ExhaustiveSearchFactors, int>(
                correctionFactors,
                limitations,
